...
int AutoRegression(float *inputseries, int length, int degree,
                   float *coefficients, int method) {
  float mean;
  int i, t;
  float *w = NULL; /* Input series - mean                            */
  float *h = NULL;
  float *g = NULL; /* Used by mempar()                              */
  float *per = NULL;
  float *pef = NULL; /* Used by mempar()                              */
  float **ar = NULL; /* AR coefficients, all degrees                  */

  /* Allocate space for working variables */
  if ((w = (float *)malloc(length * sizeof(float))) == NULL) {
...
/*
   ...
   tstarz.h no longer included, just say "typedef float REAL" instead
   Declare ar by "REAL **ar" instead of "REAL ar[MAXA][MAXA]

   Further "cleaning" by Paul Bourke.....for personal style only.
*/

int ARMaxEntropy(float *inputseries, int length, int degree, float **ar,
...
*/
int ARLeastSquare(float *inputseries, int length, int degree,
                  float *coefficients) {
  int i, j, k, hj, hi;
  float **mat;

  if ((mat = (float **)malloc(degree * sizeof(float *))) == NULL) {
    fprintf(stderr, "Unable to malloc memory - fatal!\n");
    exit(-1);
  }
  for (i = 0; i < degree; i++) {
    if ((mat[i] = (float *)malloc(degree * sizeof(float))) == NULL) {
      fprintf(stderr, "Unable to malloc memory - fatal!\n");
      exit(-1);
    }
  }
...
/*
   ...
   Modified (for personal style and context) by Paul Bourke
*/
int SolveLE(float **mat, float *vec, unsigned int n) {
  int i, j, k, maxi;
