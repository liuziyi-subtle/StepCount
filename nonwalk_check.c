...
/* _Mean */
float _Mean(const float* data, uint16_t data_length) {
...
float _Variance(const float* data, uint16_t data_length, uint16_t ddof) {
...
/* AutoCorrelation */
float _AutoCorrelation(const float* data, uint16_t data_length, uint8_t lag) {
  ...
  if (((data_length - (uint16_t)lag) * variance) == 0) {
    printf("=====================DANGEROUS!==========================\n");
  }
 ...
 * @init           initialize.
 */
uint8_t NonWalkCheck(int16_t* accx, int16_t* accy, int16_t* accz,
  ...
  if (enable == 0u) {
    ...
    for (i = 0u; i < data_length; ++i) {
      ...
      k_resultant[BUFF_LEN - data_length + i] =
          (float)(pow(accx[i], 2) + pow(accy[i], 2) + pow(accz[i], 2));
    }
  ...
  float cq_000_100_accz = _ChangeQuantile(k_accz, k_accz + BUFF_LEN, BUFF_LEN,
                                          maximum_accz, minimum_accz, true, 1u);
